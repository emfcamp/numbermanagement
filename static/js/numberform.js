...
const WORDLIST = [
  ...
  for(let i = 0; i < l; i++){
    ...
    password += word[0].toUpperCase() + word.slice(1);
  }
  return password
}
...
function toggleTOS(v){    
    ...
    switch (paramdata.param_type) {
        case "TXT":
            ...
            param.maxlength = paramdata.param_length
            param.value = null
...
function validateNumber(n){
    ...
    if (n.length == 4){
        ...
        if (!valid){
            l.innerHTML = "INVALID NUMBER RANGE" 
        }  else {
            l.innerHTML = ""
        }
    } else {
        l.innerHTML = "MUST BE 4 DIGITS" 
    }
}

//Main
const tosdata = JSON.parse(document.getElementById('tosdata').textContent);
